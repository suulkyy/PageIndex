...
- `PAGEINDEX_FAST_POOL` — candidate pool size before top-K slice (default `50`).
- `PAGEINDEX_FAST_TOP_K` — nodes whose text is fed to the synthesis call (default `8`).
- `PAGEINDEX_FAST_MAX_PER_DOC` — cap nodes per document in the top-K (default `0` = unlimited). A single lexical ranking over a multi-topic question can be monopolized by one topic's doc; set e.g. `2`–`3` to surface cross-document evidence.
...
Phase-2 semantic layer (embeddings + reranker — reuses the LightRAG `:8001`/`:8002` servers):
- `PAGEINDEX_FAST_RETRIEVER` — `auto` (default) | `bm25` | `vector` | `hybrid`. `auto` uses hybrid (BM25 + vector fused by reciprocal-rank fusion) when the index has embeddings, else BM25. Applies to fast mode and the `search_all_documents` tool.
- `PAGEINDEX_RERANK` — `true` to add a cross-encoder rerank pass over the fused pool (default `false`). On failure it logs and keeps the pre-rerank order.
- Build embeddings: `python3 build_index.py --folder ./results --embeddings` (incremental; `--reembed` after an embedding-model change; `--embed-base-url` / `--embed-model` override env). Retrieval modes do NOT auto-embed — embeddings are built only by this flag, so BM25-only retrieval never needs the servers.
- Embedding config: `PAGEINDEX_EMBED_BASE_URL` (default `http://localhost:8001/v1`), `PAGEINDEX_EMBED_MODEL` (`rag-embed`), `PAGEINDEX_EMBED_API_KEY` (`EMPTY`), `PAGEINDEX_EMBED_BATCH` (`16`), `PAGEINDEX_EMBED_TIMEOUT` (`120`).
- Rerank config: `PAGEINDEX_RERANK_BASE_URL` (default `http://localhost:8002/v1/rerank`, full path), `PAGEINDEX_RERANK_MODEL` (`rag-rerank`), `PAGEINDEX_RERANK_API_KEY` (`EMPTY`), `PAGEINDEX_RERANK_DOC_MAX_CHARS` (`2000`).
- Changing the embedding model requires `--reembed` (vectors must be recreated with the new dim; the store guards against mixed dims and a dim-mismatched query falls back to BM25).

Known limitation: lexical BM25 alone can rank one topic's document above others on a multi-topic query. Mitigations: `PAGEINDEX_FAST_MAX_PER_DOC` diversifies; the Phase-2 vector recall + rerank surface semantically-related docs that share few keywords; hybrid/agent modes' `search_all_documents` (refined sub-query) handles true multi-hop.
...
`IndexStore` builds one SQLite DB over a folder of trees: `documents` (per-doc meta), `nodes` (flattened tree with breadcrumb/parent_id/depth + node text-by-key), `nodes_fts` (standalone FTS5 mirror for BM25 across all docs, carrying `doc_id`/`node_rowid` as UNINDEXED columns so a doc's rows can be deleted on refresh), `file_manifest` (path→mtime/size/doc_id for incremental rebuilds), `index_meta`. `build(folder, reindex=False)` skips files whose mtime+size are unchanged and drops docs whose source file vanished. `search(question, top_k)` runs a recall-oriented OR'd FTS5 MATCH ranked by `bm25()` (lower = better). `get_node_content` resolves bare/unpadded node ids (weak-model tolerance, mirrors `_find_node_by_id`) and falls back to the node `summary` when the tree was built without text. `retrieve_pageindex.py:_open_index_for_folder` opens + incrementally refreshes it at startup; `build_index.py` is the offline CLI.

Phase-2 vectors (optional): `node_embeddings(node_rowid, dim, vec BLOB)` stores float32 vectors; `build_embeddings(embed_fn)` embeds `title+breadcrumb+summary` for nodes lacking a vector (incremental), with model/dim recorded in `index_meta`. `vector_search(query_vec, top_k)` does brute-force cosine over a cached, L2-normalized numpy matrix (no `sqlite-vec`/ANN — a vector DB is unnecessary at this corpus size; brute force is <10ms and the bottleneck is LLM round-trips). `retrieve_pageindex.py` fuses BM25 + vector via `_rrf_fuse` (reciprocal-rank fusion — only rank position matters, so it cleanly combines bm25's lower-is-better with cosine's higher-is-better) and optionally reranks via `pageindex/semantic.py:rerank` (`:8002`). Embeddings are built only by `build_index.py --embeddings`; the embedding/rerank servers are never required for BM25-only retrieval, and query-time embedding failures fall back to BM25.
